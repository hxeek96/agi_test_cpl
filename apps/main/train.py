...
from lingua.product_key.zero_copy import ZeroCopy
...
def train(args: TrainArgs):
    with ExitStack() as context_stack:
        ...
        # Zero_copy weight를 제거함 (GPU 이동 전)
        if hasattr(args.model, 'productkey_args') and args.model.productkey_args.zero_copy:
            logger.info("Removing Zero_copy CPU weight parameters before GPU transfer")
            cpu_embed_weights = {}
            for m in model.modules():
                if isinstance(m, ZeroCopy):
                    cpu_embed_weights[m] = m.weight
                    del m._parameters['weight']
        else:
            cpu_embed_weights = None
        ...
        # Zero_copy weight 복원 (GPU 이동 후)
        if cpu_embed_weights is not None:
            logger.info("Recovering Zero_copy CPU weight parameters after GPU transfer")
            for m, w in cpu_embed_weights.items():
    ...
    # Print training summary with Value count
    if get_is_master():
        logger.info("=" * 50)
        ...
        logger.info(f"\nTotal training steps completed: {train_state.step}")
        logger.info("=" * 50)
