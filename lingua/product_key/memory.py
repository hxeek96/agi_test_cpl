...
from .memory_offload import OffloadFpga
from .zero_copy import ZeroCopy
logger = getLogger()
...
class HashingMemory(nn.Module):
    ...
    def __init__(
        ...
        if self.original:
            if not self.use_peer_variant:  # PK
                if mem_offload:
                    self.values = OffloadFpga(self.size, self.v_dim)
                    logger.info("Using [FPGA Offload]")
                elif zero_copy:
                    self.values = ZeroCopy(self.size, self.v_dim)
                    logger.info("Using [Zero-Copy]")
                else:
                    self.values = xFormerEmbeddingBag(self.size, self.v_dim)
                    logger.info("Using [xFormer EmbeddingBag]")
    ...
    def mp_parallelize(self, mesh, model_args, distributed_args, param_dtype):
        use_fsdp = distributed_args.fsdp_type != "no_shard"
        if use_fsdp:
            fsdp_config = dict(
                mp_policy=(
    ...
    def reset_parameters(self, init_std=None, factor=1.0):
        ...
        if self.original:
            if not self.use_peer_variant:
                if not self.mem_offload:
                    # if isinstance(self.values, ZeroCopy):
                    #     self.values.reset_parameters()
                    # elif hasattr(self.values, 'weight'):
    ...
    def forward(self, input):
        ...
        if not self.training:
            self.time_stats.append(elapsed_time * 1000)  # ms 단위로 저장

            # 진행 중에는 간단한 spinner만 표시
            if len(self.time_stats) % 100 == 0:
                spinner = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏']
                idx = (len(self.time_stats) // 100) % len(spinner)
                print(f"\r[Evaluating] {spinner[idx]} Processing...", end='', flush=True)
    ...
    def __del__(self):
        """객체 소멸 시 최종 통계 출력"""
        if hasattr(self, 'time_stats') and len(self.time_stats) > 0:
            self.print_time_statistics()
    
    def print_time_statistics(self):
        """평가 종료 후 시간 통계 출력"""
        if not self.time_stats:
            print("\n[Memory Layer] No timing data collected")
            return

        stats = np.array(self.time_stats)

        # 완료 시 프로그레스 바를 쫙 채우는 애니메이션
        import sys
        bar_length = 30
        print("\r[Evaluating] ", end='', flush=True)
        for i in range(bar_length + 1):
            bar = '█' * i + '░' * (bar_length - i)
            print(f"\r[Evaluating] {bar} Complete!", end='', flush=True)
            sys.stdout.flush()
            time.sleep(0.02)  # 0.02초씩 딜레이
        print()  # 줄바꿈

        print("\n")
        print("╔" + "═" * 58 + "╗")
        print("║" + " " * 14 + "Memory Layer Performance" + " " * 20 + "║")
        print("╠" + "═" * 58 + "╣")
        print(f"║  Total Calls                : {len(stats):>10,}             ║")
        print(f"║  Avg Forward Pass Latency   : {np.mean(stats):>10.2f} ms        ║")
        print(f"║  Min Latency                : {np.min(stats):>10.2f} ms        ║")
        print("╚" + "═" * 58 + "╝")
